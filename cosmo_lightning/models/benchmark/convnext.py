...
from cosmo_lightning.models.benchmark.common import BaseModel
...
class ProjMLP(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, mlp_ratio=2):
        super().__init__()
        self.fc1 = nn.Linear(dim_in, int(dim_out * mlp_ratio))
        self.act = nn.GELU()
        self.fc2 = nn.Linear(int(dim_out * mlp_ratio), dim_out)
...
class UNetConvNext(BaseModel):
    def __init__(
        ...
        self.delta_t_project = delta_t_project
        if self.delta_t_project:
            self.start_idx_proj = ProjMLP(1,features, mlp_ratio=2)
    ...
    def forward(self, x,start_idx=None):
        x_raw = x
        x = self.in_proj(x) # N D H W
        if self.delta_t_project:
            # breakpoint()
            # start_idx is torch.long. Convert to float for arithmetic, then to x.dtype.
            processed_start_idx = (start_idx[:,None].float() - 5.0) / (12.0 - 5.0) * 3.0
            x_delta = self.start_idx_proj(processed_start_idx.to(x.dtype))[:,:,None,None] # N D
            # breakpoint()
            x = x + x_delta
        ...
        if self.predict_delta:
            x_raw = rearrange(x_raw,'n (c t) h w -> n c t h w',c=x.shape[1])
            x_raw = x_raw[:,:,-1]
            x = x + x_raw
        return x
...
if __name__ == "__main__":
    ...
    model = UNetConvNext(**config)
    model.cuda()
    x = torch.randn(1, 32, 512, 512).cuda()
    y = model(x)
    breakpoint()