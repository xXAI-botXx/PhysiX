...
# NEW IMPORT: import our custom data loader and dataset
from cosmos1.models.autoregressive.tokenizer.training.data_loader import create_dataloader, RandomHDF5Dataset
...
def train_tokenizer() -> run.Partial:
    return run.Partial(
        ...
        data=run.Config(
            HDF5DataModule,
            data_dir=os.environ.get("DATA_DIR", None),  # set via environment variable or update default as needed
            n_frames=33,
            batch_size=8,
            num_workers=1,
            dtype=torch.float32,
...
# New helper to wrap a dataset to apply the task encoder.
class TaskEncodedDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, task_encoder):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure the sample is a dict. If not, wrap it using VIDEO_KEY and MASK_KEY.
        from nemo.collections.physicalai.tokenizer.tokenizer_model import VIDEO_KEY, MASK_KEY
        if not isinstance(sample, dict):
            sample = {VIDEO_KEY: sample, MASK_KEY: torch.ones_like(sample, dtype=sample.dtype)}
        # Apply the task encoder transformation
        return self.task_encoder.encode_sample(sample)

# New LightningDataModule that wraps the autoregressive data loader.
class HDF5DataModule(pl.LightningDataModule):
    def __init__(self, data_dir, n_frames=33, batch_size=8, num_workers=1, dtype=torch.float32, data_resolution=None, task_encoder=None):
        super().__init__()
    ...
    def setup(self, stage: str = None) -> None:
        # You could add extra setup code here if needed.
        pass

    def _create_dataloader(self, shuffle: bool = True):
        # Create the basic dataloader using the provided create_dataloader function
        dataset = RandomHDF5Dataset(
            self.data_dir,
            self.n_frames,
            data_resolution=self.data_resolution,
            dtype=self.dtype,
        )
        # If a task_encoder is provided, wrap the dataset so that its __getitem__ applies the encoder.
        if self.task_encoder is not None:
            dataset = TaskEncodedDataset(dataset, self.task_encoder)
        return DataLoader(
            dataset,
    ...
    def val_dataloader(self):
        return self._create_dataloader(shuffle=False)
