...
from cosmos1.models.autoregressive.utils.inference import _SUPPORTED_CONTEXT_LEN, NUM_TOTAL_FRAMES, prepare_video_batch_for_saving, prepare_array_batch
...
def detect_model_size_from_ckpt_path(ckpt_path: str) -> str:
    ...
    """
    model_size = "4b"
    if "600M" in ckpt_path:
        model_size = "600m"
    elif "2B" in ckpt_path:
        model_size = "2b"
    elif "4B" in ckpt_path:
        model_size = "4b"
...
def create_inference_config(
    ...
    model_config, tokenizer_config = create_video2world_model_config(
        model_ckpt_path=overwrite_ckpt_path or model_ckpt_path,
...
class ARBaseGenerationPipeline(BaseWorldGenerationPipeline):
    ...
    def generate_video_from_tokens(
        ...
        video_decoded = self.model.tokenizer.video_tokenizer.decode(quant_codes.to(torch.bfloat16).cuda())
