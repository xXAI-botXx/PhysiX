...
class CustomRotaryEmbedding3D(RotaryEmbedding):
    ...
    def __init__(
        ...
        h_inv_freq = 1.0 / (self.rope_theta**self.dim_spatial_range)
        w_inv_freq = 1.0 / (self.rope_theta**self.dim_spatial_range)
        ...
        self.h_inv_freq = h_inv_freq
        self.w_inv_freq = w_inv_freq
    ...
    def get_freqs_non_repeated(self, latent_shapes: Tensor, offset: int = 0) -> Tensor:
        dtype = self.h_inv_freq.dtype
        device = self.h_inv_freq.device
        ...
        max_seq_len = max(T, H, W)
        seq = (torch.arange(max_seq_len, device=device, dtype=dtype) + offset).cuda()
        ...
        scale_factors_h = self.get_scale_factors(self.h_inv_freq, self.original_latent_shape[1])
        h_inv_freq = self.h_inv_freq * scale_factors_h
        
        scale_factors_w = self.get_scale_factors(self.w_inv_freq, self.original_latent_shape[2])
        w_inv_freq = self.w_inv_freq * scale_factors_w
        
        scale_factors_temporal = self.get_scale_factors(self.temporal_inv_freq, self.original_latent_shape[0])
        temporal_inv_freq = self.temporal_inv_freq * scale_factors_temporal
        
        half_emb_t = torch.outer(seq[:T], temporal_inv_freq.cuda())
        half_emb_h = torch.outer(seq[:H], h_inv_freq.cuda())
        half_emb_w = torch.outer(seq[:W], w_inv_freq.cuda())
        emb = torch.cat(
            [
    ...
    def forward(self, seq_len: int, latent_shapes: Tensor = None, offset: int = 0, packed_seq: bool = False) -> Tensor:
        if self.h_inv_freq.device.type == "cpu":
            # move `inv_freq` to GPU once at the first micro-batch forward pass
            self.h_inv_freq = self.h_inv_freq.to(device=torch.cuda.current_device())
        if self.w_inv_freq.device.type == "cpu":
            self.w_inv_freq = self.w_inv_freq.to(device=torch.cuda.current_device())
        if self.temporal_inv_freq.device.type == "cpu":
            self.temporal_inv_freq = self.temporal_inv_freq.to(device=torch.cuda.current_device())

        emb = self.get_freqs_non_repeated(latent_shapes)
        return emb
