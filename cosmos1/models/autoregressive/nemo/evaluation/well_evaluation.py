...
from cosmos1.models.autoregressive.nemo.evaluation.metrics import calculate_metrics
...
def main(args):
    ...
    vision_input_dict = load_vision_input(
        ...
        use_first_frames=True
    )

    vision_input = list(vision_input_dict.values())[0].cuda()
    print("Vision input shape: ", vision_input.shape)
    input_frames = vision_input[:INPUT_FRAMES]
    ground_truth = vision_input[INPUT_FRAMES:INPUT_FRAMES+PRED_FRAMES]
    ...
    quantized_out, _ = video_tokenizer.encode(input_frames, pixel_chunk_duration=None)
    ...
    if torch.cuda.current_device() == 0:
        ...
        try:
            print(calculate_metrics(ground_truth[0], output_video))
        except Exception as e:
            print(ground_truth.shape, output_video.shape)
            import traceback
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_type", type=str, default="video", help="Type of input", choices=["image", "video", "array"])
    parser.add_argument(
